...
bool NovaProximity::PowerSupplyManip(PSU& power_supply)
{
...
bool NovaProximity::NovaWindowForegroundTimeoutValidator()
{
...
bool NovaProximity::NovaWindowSetForeground()
{
...
bool NovaProximity::NovaWindowForegroundValidation()
{
...
bool NovaProximity::RTControlValidation()
{
...
void NovaProximity::Run_Reboot(PSU &power_supply)
{
	...
	uint32_t total_time = run_number * m_proximity_test_duration;
	...
	while (run_number != 0)
	{
		std::cout << (run_number + 1) % m_reboot_frequency << std::endl;
		...
		std::cout << "Starting Proximity test" << std::endl;
		StartTest();


		std::cout << "Waiting for test to complete!" << std::endl;
		for (int i = m_proximity_test_duration; i > 0; i--)
		{
			Sleep(1000);
...
bool NovaProximity::TerminateTest(const uint32_t image_counter)
{
	...
	do
	{
		...
		do
		{
			...
			try
			{
				ocr_output = screen_capture_object.ProcessImageText("result_" + std::to_string(image_counter) + ".jpg", 1);
			}
			catch (...)
			{
...
bool NovaProximity::StartTest()
{
	...
	do 
	{
		...
		ClickObject.LeftClickPos(NovaRect.left + m_rfid_proximity_test_offset_x, NovaRect.top + m_rfid_proximity_test_offset_y);
		...
		try
		{
			ocr_output = screen_capture_object.ProcessImageText("tmp.jpg", 1);
		}
		catch (...)
		{
			screen_capture_object.TakeScreenShot("tmp.jpg", 1);
...
bool NovaProximity::AnalyzeResults(const std::string& ocr_string, const std::string& validation_string)
{
...
void NovaProximity::OutputResultsLogs(const std::string output_path)
{
	std::ofstream myfile;
...
NovaProximity::NovaProximity(const Parameters& vars)
{
	m_passed_tests = 0;
	m_failed_tests = 0;
	...
	if (m_settings_vars.variables["proximity_test_duration"].compare("") == 0)
	{
		m_proximity_test_duration = 1005;
		std::cout << "Defaulting to proximity test duration of 1005 seconds" << std::endl;
	}
	else
	{
		m_proximity_test_duration = std::stoi(m_settings_vars.variables["proximity_test_duration"]);
	}
